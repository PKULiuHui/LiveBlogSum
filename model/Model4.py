...
# 配合main3.py使用的模型，将SRL预测和Sent预测分离开来
...
class Model4(nn.Module):
    ...
    def forward(self, x, doc_lens, events, event_doc_lens, event_tfs, train_srl):
        ...
        event_scores = [.0] * len(event_tfs)
        idxs = np.argsort(event_tfs.detach().cpu().numpy()).tolist()
        idxs.reverse()
        num_one = int(len(event_tfs) * 0.15)
        ...
        for i in range(0, len(doc_lens)):
            ...
            cur_event_prob = event_scores[event_start: event_start + event_doc_lens[i]]
