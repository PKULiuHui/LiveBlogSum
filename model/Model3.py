...
class Model3(nn.Module):
    def __init__(self, args, embed=None):
        ...
        self.event_salience = nn.Bilinear(2 * self.H, 6 * self.H, 1, bias=False)
        self.event_sent_pos = nn.Linear(self.P, 1, bias=False)
        self.event_doc_pos = nn.Linear(self.P, 1, bias=False)
    ...
    def forward(self, x, doc_lens, events, event_doc_lens, event_sent_lens, event_tfs):
        ...
        for i, event in enumerate(event_vec):
            ...
            event_salience = self.event_salience(event, torch.cat((sent_vec[event_sent_abs_idx[i]], doc_vec[event_doc_idx[i]], blog_vec)))
            sent_index = torch.LongTensor([[int(event_sent_idx[i] * self.args.pos_doc_size / doc_lens[event_doc_idx[i]])]])
            doc_index = torch.LongTensor([[int(event_doc_idx[i] * self.args.pos_doc_size / doc_num)]])
            if use_cuda:
                sent_index = sent_index.cuda()
                doc_index = doc_index.cuda()
            event_sent_pos = self.event_sent_pos(self.sent_pos_embed(sent_index).squeeze(0))
            event_doc_pos = self.event_doc_pos(self.doc_pos_embed(doc_index).squeeze(0))
            event_tf = self.event_tf * event_tfs[i]
            event_pre = event_content + event_salience + event_sent_pos + event_doc_pos + event_tf + self.event_bias
        ...
        for i in range(0, len(doc_lens)):
            ...
            cur_event_prob = event_probs[event_start: event_start + event_doc_lens[i]]
            ...
            for j in range(0, doc_lens[i]):
                ...
                sent_idx += 1
        return torch.cat(sent_probs).squeeze(), torch.cat(doc_probs).squeeze(), event_probs
